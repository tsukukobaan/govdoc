"""Scraper for Cabinet Secretariat (内閣官房) advisory councils."""
...
class CASScraper(BaseScraper):
    ...
    def scrape(self) -> list[dict]:
        ...
        print("  Fetching CAS seisaku master index...")
        try:
        ...
        # Collect council page links (cas.go.jp domain only)
        ...
        for a in soup.find_all("a", href=True):
            ...
            # Only cas.go.jp links (kantei.go.jp goes to kantei.py)
            if "cas.go.jp" not in full_url:
                continue
            # Links to policy/council pages under /jp/seisaku/
            if "/jp/seisaku/" not in full_url:
                continue
