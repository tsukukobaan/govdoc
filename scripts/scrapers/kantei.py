"""Scraper for Prime Minister's Office (首相官邸) advisory councils."""
from urllib.parse import urljoin

from .base import BaseScraper, normalize, parse_date


class KANTEIScraper(BaseScraper):
    ministry_slug = "kantei"
    # Uses the same CAS index page, but filters for kantei.go.jp domain links
    INDEX_URL = "https://www.cas.go.jp/jp/seisaku/index.html"
    ...
    def scrape(self) -> list[dict]:
        ...
        print("  Fetching KANTEI links from CAS seisaku index...")
        try:
        ...
        # Collect council page links (kantei.go.jp domain only)
        ...
        for a in soup.find_all("a", href=True):
            ...
            # Only kantei.go.jp links (cas.go.jp goes to cas.py)
            if "kantei.go.jp" not in full_url:
                continue
        ...
        print(f"  Found {len(council_pages)} kantei council page links")
